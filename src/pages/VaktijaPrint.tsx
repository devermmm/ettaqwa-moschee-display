  ...
  return (
    <>
      ...
      <div className="print-page bg-white mx-auto flex flex-col" style={{ width: "210mm", minHeight: "297mm", fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
        <div className="flex-1 m-[5mm] border-[3px] border-emerald-800 rounded-lg p-[3mm] flex flex-col">
          <div className="flex-1 border border-emerald-600 rounded p-[3mm] flex flex-col">

            {/* Header */}
            <div className="text-center mb-1">
              <img src={logo} alt="Et-Taqwa" className="mx-auto h-12 mb-0.5" />
              <p className="text-base leading-snug font-bold text-gray-800" style={{ fontFamily: "'Amiri', 'Traditional Arabic', serif", direction: "rtl" }}>
            ...
            <div className="text-center mb-1.5">
              <h1 className="text-xl font-black text-emerald-800 tracking-wide">VAKTIJA</h1>
              <p className="text-[10px] text-gray-700 font-semibold">2025/1446. (28. februar - 29. mart)</p>
            </div>
              ...
              <table className="w-full border-collapse text-[10px] leading-tight">
                <thead>
                  <tr className="bg-emerald-800 text-white text-[9px]">
                    <th className="border border-emerald-700 px-1.5 py-1.5 font-bold text-center" style={{width:"5%"}}>R.</th>
                    <th className="border border-emerald-700 px-1 py-1.5 font-bold text-center" style={{width:"5%"}}></th>
                  ...
                  {ramadanDays.map((day) => {
                    ...
                    return (
                      <tr
                        key={day.ramazanDay}
                        className={`
                          ${isEid ? "bg-amber-100 font-bold" : isFriday ? "bg-emerald-50 font-semibold" : day.ramazanDay % 2 === 0 ? "bg-gray-50" : "bg-white"}
                        `}
                      >
                        <td className="border border-gray-300 px-1.5 py-[3px] text-center font-bold text-emerald-800">{day.ramazanDay}</td>
                        <td className={`border border-gray-300 px-1 py-[3px] text-center ${isFriday ? "text-emerald-700 font-bold" : "text-gray-600"}`}>{day.dayName}</td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-semibold">{day.gDay}</td>
                        <td className={`border border-gray-300 px-1.5 py-[3px] text-left text-[9px] ${event ? "text-emerald-800 font-semibold" : "text-gray-400"}`}>
                          {event || ""}
                        </td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-mono text-emerald-900 font-bold">{pt?.fajr || ""}</td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-mono text-gray-600">{pt?.sunrise || ""}</td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-mono">{pt?.dhuhr || ""}</td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-mono">{pt?.asr || ""}</td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-mono text-emerald-900 font-bold">{pt?.maghrib || ""}</td>
                        <td className="border border-gray-300 px-1 py-[3px] text-center font-mono">{pt?.isha || ""}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            {/* Bajram Notice */}
            <div className="text-center mt-1.5 bg-emerald-50 border border-emerald-300 rounded py-1.5 px-2">
            ...
            <div className="flex items-center justify-between mt-1.5 px-2">
              <span className="text-2xl font-black text-emerald-800">2025.</span>
              ...
              <span className="text-2xl font-black text-emerald-800">1446.</span>
            </div>

            {/* Mosque Listings */}
            <div className="mt-1.5 pt-1.5 border-t-2 border-emerald-800">
